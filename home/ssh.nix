        ...
        keep-agent-alive = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            exec autossh \
              ...
              -A "$server" -- \
              'echo "Agent forwarding established." && while true; do if ! [ -e "$SSH_AUTH_SOCK" ]; then echo "Authentication socket $SSH_AUTH_SOCK does not exist anymore."; exit 1; else echo "Auth sock OK"; fi; sleep 10; done'
          '';
        };
      in
      {
