...
in
{
  config = mkMerge [
          ...
          peers = [
            {
              ...
              endpoint = ahrefsEndpoint;
            }
          ];
        };
      };

      ## wstunnel client for tunnelled mode. Not started automatically; use
      ## `ahrefs-vpn-tunnelled` to start it and switch the WireGuard endpoint.
      services.wstunnel.clients.ahrefs-vpn = {
        enable = true;
        autoStart = false;
    ...
    (mkIf ahrefs-vpn-tunnel.enabledOnThisServer {
      services.wstunnel.servers.ahrefs-vpn = {
        enable = true;
        listen = {
          host = "127.0.0.1";
