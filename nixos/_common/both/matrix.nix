...
in
{
  config = mkMerge [
    (mkIf config.x_niols.services.matrix.enabledOnAnyServer {
      ...
      ##
      ## These well-known entries might be served by a different machine. `server`
      ## is for federation and `client` is for Matrix clients.
      ##
    ...
    (mkIf config.x_niols.services.matrix.enabledOnThisServer {
      services.postgresql = {
        ensureUsers = [ { name = "matrix-synapse"; } ];
        ## NOTE: Database `matrix-synapse` has to be created manually. Be
        ## careful with the collation. Refer to the documentation:
