...
let
  ...
  commonModule =
    { config, osConfig, ... }:
    {
      ...
      ## Inject existing secrets into the configurations. Agenix will try to
      ## decrypt all secrets that are in `config.age.secrets`, so we need to
      ## filter out the ones that the machine will not be able to decrypt.
